...
- `Client::metric_metadata()` now returns a builder object (similar to `Client::query` and `Client::query_range`). This object is then used to optionally set filters to a query to the metric metadata endpoint before sending it with `MetricMetadataQueryBuilder::get()`. This "builder scheme" avoids large (and potentially growing) function signatures on `Client::metric_metadata()`.
